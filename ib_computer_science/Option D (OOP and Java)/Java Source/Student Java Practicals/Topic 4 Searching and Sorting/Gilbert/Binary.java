...
public class Binary {
	public static void selectionSort(int[] arr) {
		/**
		 * method selectionSort performs selection sort on given array for binary searching
		 * input :
		 * int arr[] : list of integers that are to be sorted
		 * output : 
		 * there is no output in this method, it is an in-place function to perform a selection sort
		 **/
  ...
  public static int[] binarySearch(int arr[], int startIndex, int endIndex, int searchItem){
	  /**
	   * method binarySearch performs binary search recursively on a given array arr.  
	   * input : 
	   * int arr[] : list of integers to perform binary search on
	   * int startIndex : the first index which would generally be 0 when input
	   * int endIndex : the last index of the array, which would be arr.length-1 when input
	   * int searchItem : the item being searched
	   * output : 
	   * int result[] : array containing the index and the value being searched when the variable is found,
	   * else contains [-1,-1].
	   **/
